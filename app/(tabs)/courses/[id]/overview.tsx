...
export default function CourseOverview() {
  ...
  // ── helper: is this user the course owner? ──────────────────────────────
  const isCourseOwner = isInstructor && course?.createdUserId === user?.id;
  ...
  useEffect(() => {
    ...
    const playSub = player.addListener('playingChange', (event) => {
      ...
      if (event.isPlaying) {
        ...
        // ✅ Only save progress if NOT course owner
        if (!isCourseOwner) {
          saveProgress(false);
        }
      }
    ...
    // ✅ video end – instructors do NOT trigger completion
    const endSub = player.addListener('playToEnd', async () => {
      ...
      // ✅ Only students save progress and complete lessons
      if (!isCourseOwner) {
        ...
        console.log('[Lesson] Skipping completion - instructor');
      }
    });
  ...
  }, [player, currentLesson, course, user]);
  ...
  // Save progress when screen unmounts (only for students)
  useEffect(() => {
    return () => {
      if (timeInterval.current) {
        clearInterval(timeInterval.current);
  ...
  const initTracker = async () => {
    // ✅ Instructors never create trackers
  ...
  const saveProgress = async (videoFinished: boolean) => {
    // ✅ Instructors never save progress
    if (isCourseOwner) return;
  ...
  const loadExistingTracker = async (lessonId: string) => {
    // ✅ Instructors never load trackers
    if (isCourseOwner) return;
    try {
  ...
  // Handle lesson completion (ONLY for students)
  const handleCompleteLesson = async (lessonId: string) => {
    if (isCourseOwner) return; // extra safety
    if (!token || !params.id) {
  ...
  const handlePlayPause = async () => {
    if (!player) return;
    if (!isPlaying) {
      // ✅ If instructor, just play – no tracker init
      if (!isCourseOwner) {
        ...
        console.log('[Instructor] Playing without tracking');
      }
      player.play();
  ...
  const handleLessonSelect = async (lesson: any) => {
    // ✅ If instructor, skip saving progress
    if (!isCourseOwner && currentTrackerId.current) {
  ...
  const totalLessons =
    course.module?.reduce((s: number, m: any) => s + (m.lesson?.length || 0), 0) || 0;
  const totalDuration =
    course.module?.reduce((s: number, m: any) => s + (parseInt(m.module_duration) || 0), 0) || 0;
    ...
    <MenuProvider>
      <SafeAreaView style={s.container} edges={['top']}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          {/* ✅ VIDEO PLAYER FOR BOTH INSTRUCTORS AND STUDENTS */}
          ...
          <View style={s.tabs}>
            ...
            <TouchableOpacity
              style={s.tab}
              onPress={() => router.push(`/(tabs)/courses/${params.id}/quizzes` as any)}
            >
              <Text style={s.tabText}>{t('courses.tabQuizzes')}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={s.tab}
              onPress={() => router.push(`/(tabs)/courses/${params.id}/materials` as any)}
            >
              <Text style={s.tabText}>{t('courses.tabMaterials')}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={s.tab}
              onPress={() => router.push(`/(tabs)/courses/${params.id}/forums` as any)}
            >
